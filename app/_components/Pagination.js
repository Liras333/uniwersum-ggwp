...
export default function Pagination({ count }) {
  ...
  if (maxPage + 1 === 0)
    return (
      <div className="flex flex-col h-full justify-center items-center text-neutral-300 ">
        <span className="">brak danych</span>
      </div>
    );
  ...
  return (
    <div className="flex flex-col justify-center items-center text-neutral-300">
      <div className="py-5  flex items-center justify-center gap-2">
        <button
          disabled={isPending}
          className="cursor-pointer hover:text-neutral-200 disabled:text-neutral-700 disabled:cursor-wait"
          onClick={prev}
        >
          <ChevronLeft />
        </button>
        <span>
          Strona {aktywnaStrona} - {maxPage + 1}
        </span>
        <button
          disabled={isPending}
          className="cursor-pointer hover:text-neutral-200 disabled:text-neutral-700 disabled:cursor-wait"
