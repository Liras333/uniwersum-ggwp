...
export default async function ListaSwiatow({swiaty}){
    ...
    return (
        swiaty.map((swiat) => (
            ...
            <Link  href={`/archiwa/swiaty/${swiat.idSwiata}`} key={`${swiat.idSwiata} swiat`} className="grid grid-cols-1 grid-rows-2 lg:grid-cols-[auto_1fr] lg:grid-rows-1 justify-items-center items-center gap-5 p-5  hover:bg-secondary-800 transition-colors rounded">
                <div className="relative  h-40 w-70">
                    {swiat?.obraz
                    ? <Image fill  className="object-cover" src={swiat.obraz?.[0]}  alt={`obraz ${swiat.nazwa}`}/>
